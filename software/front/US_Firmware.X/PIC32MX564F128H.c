...
void mcuInit1(){
    ...
    TRISB = 0b1111000000111111;
    TRISC = 0b1001000000000000;
    TRISD = 0b0000011100100000;
    TRISE = 0;
    TRISF = 0;
    TRISG = 0b1000111111;
...
void mcuInit2(){
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 10);
    OLEDUpdate();}
    
    OpenTimer3(T3_ON | T3_IDLE_STOP | T3_GATE_OFF | T3_PS_1_256 | T3_SOURCE_INT, 40);
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 11);
    OLEDUpdate();}
    
    OpenOC1(OC_OFF | OC_IDLE_STOP | OC_TIMER_MODE16 | OC_TIMER3_SRC | OC_PWM_FAULT_PIN_DISABLE, 0x20, 0);
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 12);
    OLEDUpdate();}
    
    DmaEnable(1);
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 13);
    OLEDUpdate();}
    
    I2CEnable(I2C4, FALSE);
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 14);
    }
    
    mcuI2CReset();
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 15);
    OLEDUpdate();}
    
    I2CSetFrequency(I2C4, PER_FREQ, 400000);
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 16);
    OLEDUpdate();}
    
    I2CEnable(I2C4,TRUE);
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 17);
    OLEDUpdate();}

    CVREFOpen(CVREF_ENABLE | CVREF_OUTPUT_DISABLE | CVREF_RANGE_HIGH | CVREF_SOURCE_VREF);
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 18);
    OLEDUpdate();}
    ...
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 19);
    OLEDUpdate();}
        
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 20);
    OLEDUpdate();}
    ...
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 21);
    OLEDUpdate();}
}
...
void mcuInit3(){
    ...
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 22);
    OLEDUpdate();}
}
...
void mcuInit4(){
    ...
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 41);
    OLEDUpdate();}
    ...
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 42);
    OLEDUpdate();}
    
    INTEnableSystemMultiVectoredInt();
    if(numbers_adv_view==1){OLEDPrintNum816(0, 0, 2, 43);
    OLEDUpdate();}
}
...
void mcuADCStartManualVRef(){
    ...
    OpenADC10(\
            ...
            ENABLE_AN0_ANA | ENABLE_AN1_ANA | ENABLE_AN2_ANA | ENABLE_AN3_ANA | ENABLE_AN4_ANA | ENABLE_AN5_ANA | ENABLE_AN12_ANA | ENABLE_AN13_ANA | ENABLE_AN14_ANA, \
            SKIP_SCAN_ALL);
